...
const pageAccess = {
    ...
    'offers.html': 'offers',
    'my-account.html': 'my-account'  // Everyone has access to their own account
};

// Get users from localStorage (managed by Super Admin)
function getManagedUsers() {
    const usersStr = localStorage.getItem('freezyBiteUsers');
...
// Get user access permissions (checks managed users first)
...
function isLoggedIn() {
    return localStorage.getItem('freezyBiteLoggedIn') === 'true' && getCurrentUser() !== null;
}
...
function hasAccess(pageName) {
    ...
    if (user.access.includes('all')) return true;
    
    const pageKey = pageAccess[pageName];
    return user.access.includes(pageKey);
}
...
function checkAuth() {
    ...
    if (!hasAccess(currentPage)) {
        ...
        } else if (user.access.includes('overview')) {
            window.location.href = 'index.html';
        } else if (user.access.includes('products')) {
