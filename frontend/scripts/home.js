...
(function(){
  ...
  // Prevent default submit for demo, show toast
  panels.forEach(function(p){
    p.addEventListener('submit', function(e){ e.preventDefault(); showToast('Signed in (demo)'); closeModal(); });
  });

  var googleBtn = doc.querySelector('.social-google');
  var fbBtn = doc.querySelector('.social-facebook');
  if (googleBtn) googleBtn.addEventListener('click', function(){ showToast('Continue with Google (demo)'); });
  if (fbBtn) fbBtn.addEventListener('click', function(){ showToast('Continue with Facebook (demo)'); });
